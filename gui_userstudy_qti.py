...
class App(QWidget):
    def __init__(self, user, batch_size):
        super().__init__()
        self.user = user

        save_file = 'userstudy_log/{}_IoU.npy'.format(self.user)
        if os.path.isfile(save_file):
            self.np_IoU = np.load(save_file)
            print('Save File Loaded for {}'.format(self.user))
            print('------------------Maximum score -----------------------')
            for i in range(13):
                print('{:20} | JF: {:.3f} / {:.3f}'.format(SO_SET[i], np.max(self.np_IoU[i,:,0]), np.max(self.np_IoU[i,:,1])))
            print('-------------------------------------------------------')
        else:
            print('No Saved File for {}'.format(self.user))
            self.np_IoU = np.zeros((13,8,2))
        # print
        ...
        self.report_button = QPushButton('SAVE')
        ...
        navi.addWidget(self.report_button)
    ...
    def re_init(self, sequence):
        ...
        # init IoU
        # self.np_IoU[SO_SET.index(sequence),:,:] = 0
        self.this_round = -1
        ...
        # set window
        self.setWindowTitle('Demo: STM interactive - User: {} / Seq: {}'.format(self.user, self.sequence))
    ...
    def on_report(self):
        ment_J = 'Saving_J -- {} | idx: {} | U: {} | R: {} | J: [ '.format(self.sequence, SO_SET.index(self.sequence), self.user, self.this_round)
        ment_F = 'Saving_J -- {} | idx: {} | U: {} | R: {} | F: [ '.format(self.sequence, SO_SET.index(self.sequence), self.user, self.this_round)
        for r in range(8):
            Jscore = self.np_IoU[SO_SET.index(self.sequence),r,0]
            Fscore = self.np_IoU[SO_SET.index(self.sequence),r,1]
            if Jscore > 0:
                ment_J += '{:.3f}, '.format(Jscore)
                ment_F += '{:.3f}, '.format(Fscore)
        ment_J = ment_J[:-2] + ' ]'
        ment_F = ment_F[:-2] + ' ]'
        print(ment_J)
        print(ment_F)
        np.save(os.path.join('userstudy_log', '{}_IoU'.format(self.user)), self.np_IoU)

        print('-----------Current Maximum score -----------------------')
        for i in range(13):
            print('{:20} | JF: {:.3f} / {:.3f}'.format(SO_SET[i], np.max(self.np_IoU[i,:,0]), np.max(self.np_IoU[i,:,1])))
        print('-------------------------------------------------------')
    ...
    def on_reset(self):
        self.np_IoU[SO_SET.index(self.sequence),:,:] = 0
        self.re_init(self.sequence)
    ...
    def on_run(self):
        ...
        # print(sequence, pred_masks.shape, gt_masks.shape)
        Jscore = batched_jaccard(gt_masks, pred_masks)
        Fscore = batched_f_measure(gt_masks, pred_masks)
        ment = 'Round {} - {}  | U: {} | JF:[{:.3f}, {:.3f}]'.format(self.this_round+1, self.sequence, self.user, np.mean(Jscore), np.mean(Fscore))
        print(ment)

        self.np_IoU[SO_SET.index(self.sequence), min(self.this_round,7), 0] = np.mean(Jscore)
        self.np_IoU[SO_SET.index(self.sequence), min(self.this_round,7), 1] = np.mean(Fscore)
...
if __name__ == '__main__':
    ...
    def get_arguments():
        ...
        parser.add_argument('-u',  type=str, help="User name", required=False, default='user1')
